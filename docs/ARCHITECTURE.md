...
```mermaid
flowchart TD
    A["ГЦСТ, локальні TXT або raw-архів"] --> B["Raw-файли та декодування"]
    B --> C["SQLite v4 та контроль якості"]
    C --> D["Оригінал або активна правка"]
    D --> E["Панель рівнів"]
    D --> F["DOCX, карта і графіки"]
    C --> G["Довідник екстремумів"]
    G --> F
```
...
```mermaid
flowchart TD
    P["DOCX або PNG"] --> L["Зв'язок і знімок значення"]
    L --> O["Спостереження"]
    O --> I["Імпорт"]
    I --> R["Raw-файл"]
    L --> C["Правка, якщо застосована"]
```
...
- `NOT_CHECKED` — немає попереднього рівня для перевірки добової зміни;
...
«Без зауважень», «Дані відсутні», «Не перевірено», «Підозріле значення»,
«Поза діапазоном», «Неузгоджена зміна» та «Виправлено».
...
Вісь часу графіка рівнів закінчується останнім наявним спостереженням:
якщо за останню дату є лише 08:00, порожній термін 20:00 не додається до
видимого діапазону. Наявні вечірні значення та внутрішні пропуски
зберігаються. Облік пропусків у метаданих охоплює весь вибраний період.

Межі вертикальної осі витрат визначаються за наявними значеннями з
відступами. Для невід'ємного ряду нижня межа не опускається нижче нуля.
Фактичні від'ємні значення із зауваженнями QC не обрізаються.

## Поля вибору та прокручування

Поля поста, джерела та сервера мають обмежену ширину. Початок і кінець
періоду розміщуються поруч, а за нестачі місця — у два рядки. Календар
залишається біля відповідної дати.

Коліщатко над закритим списком прокручує головну форму, не змінюючи вибір.
Відкритий список зберігає стандартне прокручування й клавіатурне керування.
...
У one-folder-збірці PyInstaller бібліотеки, шаблони, карта, шрифт і
конфігурація розміщуються в `_internal`. SQLite, raw-архів, `.env` і готові
матеріали створюються поруч із `HydroBulletin.exe`. Передавати потрібно
всю папку програми. `.env` і робочі дані до дистрибутива не входять.
