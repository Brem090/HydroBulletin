"""Тести декодера, джерел даних та початкового архіву HydroBulletin."""
...
# Тести декодування рівнів води

class ParseLevelTests(unittest.TestCase):
    """Тести декодування рівнів води."""

    def test_positive_level(self) -> None:
        """Перевірка декодування додатного рівня води."""
        self.assertEqual(parse_level("10214"), 214)

    def test_negative_level(self) -> None:
        """Перевірка декодування від'ємного рівня води."""
        self.assertEqual(parse_level("15003"), -3)

    def test_invalid_level(self) -> None:
        """Перевірка обробки некоректного коду рівня."""
        self.assertIsNone(parse_level("30214"))


# Тести декодування вечірніх рівнів води

class ParseEveningLevelTests(unittest.TestCase):
    """Тести декодування вечірніх рівнів води."""

    def test_positive_evening_level(self) -> None:
        """Перевірка декодування додатного вечірнього рівня."""
        self.assertEqual(parse_evening_level("30208"), 208)

    def test_negative_evening_level(self) -> None:
        """Перевірка декодування від'ємного вечірнього рівня."""
        self.assertEqual(parse_evening_level("35003"), -3)

    def test_invalid_evening_level(self) -> None:
        """Перевірка обробки некоректного коду вечірнього рівня."""
        self.assertIsNone(parse_evening_level("10208"))


# Тести декодування добової зміни рівня води

class ParseChangeTests(unittest.TestCase):
    """Тести декодування добової зміни рівня води."""

    def test_rise(self) -> None:
        """Перевірка декодування підйому рівня."""
        self.assertEqual(parse_change("20061"), 6)

    def test_fall(self) -> None:
        """Перевірка декодування спаду рівня."""
        self.assertEqual(parse_change("20042"), -4)

    def test_no_change(self) -> None:
        """Перевірка декодування відсутності зміни рівня."""
        self.assertEqual(parse_change("20000"), 0)

    def test_invalid_sign_code(self) -> None:
        """Перевірка обробки некоректного коду зміни рівня."""
        self.assertIsNone(parse_change("20069"))


# Тести декодування одного запису гідропоста

class StationRecordTests(unittest.TestCase):
    """Тести декодування окремого запису гідропоста."""

    def test_decode_record(self) -> None:
        """Перевірка декодування повного запису гідропоста."""
    ...
    def test_nil_record(self) -> None:
        """Перевірка обробки запису з відсутніми даними (NIL)."""
...
# Тести декодування гідрологічних кодів

class DecodeCodesTests(unittest.TestCase):
    """Тести декодування файлів із гідрологічними кодами."""

    def test_filters_other_day(self) -> None:
        """Перевірка відбору даних лише за потрібну дату."""
        raw_text = """
...
"""
    ...
    def test_returns_known_station_data(self) -> None:
        """Перевірка правильності декодування відомого гідропоста."""
        raw_text = "81017 12081 10214 20061 30208 ="
...
# Тести локального джерела даних

class LocalFileSourceTests(unittest.TestCase):
    """Тести роботи локального джерела даних."""

    def test_reads_utf8_file(self) -> None:
        """Перевірка читання UTF-8 файлу."""
    ...
    def test_missing_file_raises_error(self) -> None:
        """Перевірка винятку при відсутності файлу."""
        with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    def test_online_settings_contain_only_variable_names(self) -> None:
        """Перевірка безпечних налаштувань онлайн-джерела."""
...
# Тести локального SQLite-архіву

class ArchiveTests(unittest.TestCase):
    """Тести створення та ініціалізації локального архіву."""

    def test_initialize_archive_creates_database_and_stations(self) -> None:
        """Перевірка створення бази даних та довідника гідропостів."""
        with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    def test_initialize_archive_can_run_twice(self) -> None:
        """Перевірка повторної ініціалізації архіву без дублювання даних."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "hydro_archive.sqlite"
